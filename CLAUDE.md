...
- **data-migrator** — только M1: переносит содержимое `SNP Е2Е процесс.pptx` в `src/data/snp/process.json` через `scripts/import-pptx.py` (python-pptx), затем расставляет координаты `scripts/layout.ts`. Обязан сверить количество шагов с презентацией и записать список id в `tests/fixtures/required-nodes.json`.
