...
- **data-migrator** — переносит содержимое презентации карты (реестр `MAPS` в импортёре) в `src/data/<карта>/process.json` через `scripts/import-pptx.py` (python-pptx), затем расставляет координаты `scripts/layout.ts`. Обязан сверить количество шагов с презентацией и записать список id в `tests/fixtures/<id>/required-nodes.json`.
...
- Не изобретать процесс: содержание этапов и шагов — только из презентации и process.json; неясности — задача с `-t decision`, а не догадка (типа `question` в bd нет, `bd create -t question` падает).
- Не добавлять бэкенд, роутер, Tailwind, UI-киты.
  Переключатель версий карты (`process-map-0c5`) роутером не является и этому правилу не противоречит: адрес страницы он не меняет, экранов не заводит, история браузера не растёт (`replaceState`). Он выбирает, из какого из двух JSON, УЖЕ лежащих в бандле этой страницы, читать данные. Страница по-прежнему выбирается на сборке переменной `MAP`, и разные процессы по-прежнему живут на разных адресах.
- Не менять `design/`.
