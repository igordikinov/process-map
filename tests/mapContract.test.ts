...
describe.each(maps)('контракт карты: $id', ({ id, source }) => {
  ...
  it('ровно четыре этапа с номерами 1..4 без повторов', () => {
    // Схема допускает только number ∈ {1,2,3,4} (schema.ts), но не требует,
    // чтобы этапов было четыре и чтобы номера не повторялись.
    expect(map.stages).toHaveLength(4);
    expect(map.stages.map((stage) => stage.number).sort()).toEqual([1, 2, 3, 4]);
  });
  ...
  it('каталог назван так же, как источник данных', () => {
    // Сегодня это проверка формы имени каталога. С появлением поля id в схеме
    // (задача process-map-3wh.4) сюда добавится сверка map.id === id: тогда
    // ключ localStorage, подпись рамки и содержание перестанут иметь
    // возможность разъехаться.
