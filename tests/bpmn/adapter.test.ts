...
const MODEL_FACTS = {
  ...
  /*
   * Подпись карточки — код И название (process-map-ax5). Одним кодом было
   * восемь карточек из десяти подписаны двумя-тремя буквами, а видимая подпись
   * расходилась с тем, что произносит скринридер.
   *
   * Восьмая запись без кода не случайна: у модуля MRP имя и код совпадают, и
   * «MRP · MRP» читалось бы как дефект отрисовки.
   */
  shortTitles: [
    'DP · Планирование спроса',
    'SNP · Планирование сети поставок',
    'PS · Детальное планирование производства и Графикование',
    'DM · Управление данными',
    'IO · Inventory Optimization',
    'SOP · Процесс S&OP',
    'TPP · Trade-Promo Planning',
    'MRP',
...
describe('модель In.Plan v11: содержание', () => {
  ...
  /*
   * ВИДИМАЯ ПОДПИСЬ И ДОСТУПНОЕ ИМЯ ОБЯЗАНЫ СОВПАДАТЬ (process-map-ax5).
   *
   * `shortTitle` рисуется на карточке, `title` уходит в подсказку, в крошки и в
   * aria-label. Разойдись они — зрячий видел бы «DP», а скринридер произносил
   * «Планирование спроса», и сослаться на одну карточку они бы не смогли. Это
   * нарушение WCAG 2.5.3 «Label in Name», и оно тут уже было.
   */
  it('подпись карточки совпадает с доступным именем этапа', () => {
    for (const stage of map.stages) {
      expect(stage.shortTitle, `этап ${stage.number}`).toBe(stage.title);
    }
  });

  /*
   * Обзор из десяти карточек, подписанных двумя-тремя буквами, прочесть нельзя.
   * Проверяется не длина ради длины, а то, что подпись содержит СЛОВА, а не
   * только код: код без названия — это и был дефект.
   */
  it('подпись карточки называет процесс, а не только код модуля', () => {
    const codeOnly = map.stages.filter((stage) =>
      /^[A-ZА-ЯЁ][A-ZА-ЯЁ0-9&]{1,7}$/u.test(stage.shortTitle),
    );
    // Единственное исключение — модуль, у которого имя и есть код (MRP).
    expect(codeOnly.map((stage) => stage.shortTitle)).toEqual(['MRP']);
  });
  ...
  /*
   * ДУБЛИ ВЫХОДОВ (process-map-xsk). В этой модели одинаково названные объекты
   * данных есть в шести модулях из десяти. Карточка обзора печатала такой выход
   * дважды, а React на повторяющемся ключе предупреждал, что элементы могут
   * дублироваться ИЛИ ПРОПАДАТЬ.
   *
   * Проверка на настоящей модели, а не на синтетике: дубли — свойство этого
   * файла, и тест обязан краснеть именно на нём.
   */
  it('ключевые выходы этапа не повторяются', () => {
    for (const stage of map.stages) {
      expect(new Set(stage.keyOutputs).size, `дубль в этапе «${stage.title}»`).toBe(
        stage.keyOutputs.length,
      );
    }
  });

  /*
   * ВТОРАЯ ПОЛОВИНА ТОГО ЖЕ ДЕФЕКТА, И ОНА ТИШЕ ПЕРВОЙ. Дедупликация обязана
   * идти ДО среза до четырёх. Если срезать первыми, дубль занимает слот и
   * вытесняет из четвёрки выход, который больше нигде не назван, — потеря
   * содержания без единого признака на экране.
   *
   * Формула ниже ловит именно порядок: при срезе до дедупликации у этапа с
   * дублем в первой четвёрке выходов останется меньше четырёх.
   */
  it('дедупликация идёт до среза: четвёрка заполнена, пока есть чем', () => {
    for (const stage of map.stages) {
      ...
      expect(stage.keyOutputs.length, `этап «${stage.title}»`).toBe(Math.min(4, distinct));
    }
  });
