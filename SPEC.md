...
- `tests/snp/` — **содержание карты SNP**: то, что не обязано быть верным про другую карту. `content.test.ts` (объём ≥ 40 узлов, обязательные id из `tests/fixtures/snp/required-nodes.json`, входы и выходы у каждого этапа — регрессия 24p), `updatedAt.test.ts` (дата и отпечаток), `importPreserve.test.ts` (решения владельца в импортёре).
