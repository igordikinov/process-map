...
// стиля.
//
// Здесь проверяется ГЕОМЕТРИЯ: карточки этапов реально внутри рамки. Подпись
// рамки проверяет e2e/maps/smoke.spec.ts — она разная у разных карт, и её
// литерал в спеке, который гоняется только для SNP, был бы дублем
// (process-map-3wh.3).
test('карточки этапов лежат внутри рамки потока', async ({ page }) => {
