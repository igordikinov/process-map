 ...
 *
 * ЗНАЧЕНИЕ ИСПРАВЛЕНО ПО НАСТОЯЩЕМУ ФАЙЛУ (process-map-70e.1). Прежние 4 MB
 * обосновывались размером `src/data/snp/process.json` (69 KB) — не тем
 * ориентиром: модель In.Plan v11 весит 1.29 MB, то есть запас был не «почти
 * порядок», а всего троекратный, и следующая версия модели упёрлась бы в
 * потолок. Восемь мегабайт — примерно шестикратный запас к реальному файлу.
 * Разбор такого объёма занимает доли секунды; от подвешивания вкладки защищает
 * не этот лимит, а потолок числа узлов после разбора.
 */
export const MAX_BPMN_BYTES = 8 * 1024 * 1024;

/**
 * Потолок числа элементов XML — дешёвый предфильтр против абсурдного файла.
 *
 * ЗНАЧЕНИЕ ИСПРАВЛЕНО ПО НАСТОЯЩЕМУ ФАЙЛУ (process-map-70e.1). Прежние 20 000
 * были взяты с потолка и оказались опасно близко: модель In.Plan v11 содержит
 * 15 725 элементов, то есть 79% лимита — обычный рост модели отклонял бы
 * честный файл с формулировкой «слишком много элементов».
 *
 * От подвешивания вкладки защищает не эта проверка, а потолок числа узлов
 * ПОСЛЕ разбора: 911 узлов потока в файле превращаются в карту, и опасен
 * именно размер карты, а не размер XML.
 */
export const MAX_BPMN_ELEMENTS = 120000;
