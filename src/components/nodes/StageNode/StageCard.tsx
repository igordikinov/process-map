    ...
    >
      ...
      <ul className={styles.outputs}>
        {outputs.map((output, index) => (
          ...
          //
          // Ключ по ИНДЕКСУ, а не по тексту (process-map-xsk). Схема допускает
          // одинаковые строки в keyOutputs (z.string().array(), без проверки
          // уникальности), и на карте из BPMN они реально встречались. React на
          // повторяющемся ключе предупреждает, что элементы могут дублироваться
          // ИЛИ ПРОПАДАТЬ, — то есть карточка молча теряла бы выход. Список
          // статичен и не переупорядочивается, поэтому индекс здесь безопасен, а
          // главное — не зависит от свойства данных, которого схема не требует.
          <li key={index} className={styles.output}>
